...
def android_user_agent_all_devices(seed=None):
    if seed:
        random.seed(seed)
    ...
    FBAV = f"{random.randint(420, 460)}.0.0.{random.randint(10, 90)}.{random.randint(80, 250)}"
    FBBV = str(random.randint(20000000, 99000000))

    density = random.choice(["2.0","2.5","3.0","3.5"])
    width = random.choice(["720","1080","1440"])
    height = random.choice(["1600","1920","2400","2560","2800"])

    FBLC = random.choice(["bn_BD","en_US","en_GB","en_IN","en_SG"])
    FBCR = random.choice([
        "Grameenphone","Robi","Banglalink","Airtel","Teletalk",
        "Jio","Airtel IN","Vodafone","T-Mobile","AT&T","Verizon",
        "Telkomsel","Digi","Maxis"
    ])

    devices = [
        # ---------------- Samsung A/M Series 2024–2025 ----------------
        "Galaxy A03","Galaxy A04","Galaxy A05",
        "Galaxy A13","Galaxy A14","Galaxy A15",
        "Galaxy A24","Galaxy A25","Galaxy A34",
        "Galaxy A54","Galaxy A55",
        "Galaxy M14","Galaxy M33","Galaxy M34",

        # ---------------- Samsung Flagship ----------------
        "Galaxy S20","Galaxy S21","Galaxy S21 FE",
        "Galaxy S22","Galaxy S23","Galaxy S23 FE",
        "Galaxy S24","Galaxy S24+","Galaxy S24 Ultra",
        ...
        "Poco F5","Poco F6","Poco F6 Pro",

        # ---------------- Realme ----------------
        "Realme C33","Realme C51","Realme C55","Realme C67",
        "Realme Narzo 50","Realme Narzo 60","Realme 10 Pro",
        "Realme 11 Pro","Realme 12 Pro","Realme GT Neo 3","Realme GT Neo 5",

        # ---------------- Oppo ----------------
        "Oppo A57","Oppo A58","Oppo A78","Oppo A79","Oppo A98",
        "Oppo F19 Pro","Oppo F21 Pro","Oppo Reno 8","Oppo Reno 10",

        # ---------------- Vivo ----------------
        "Vivo Y21","Vivo Y22","Vivo Y33s","Vivo Y36","Vivo Y100",
        "Vivo V25e","Vivo V27e","Vivo V29e",
        ...
        # ---------------- Google Pixel ----------------
        "Pixel 5a","Pixel 6","Pixel 6a","Pixel 7","Pixel 7a","Pixel 8","Pixel 8a",

        # ---------------- Infinix / Tecno ----------------
        "Infinix Hot 30","Infinix Hot 40","Infinix Hot 40 Pro",
        "Infinix Note 30","Infinix Note 40 Pro",
    ...
    brand_key = device.split()[0]
    FBMF = manufacturers.get(brand_key, "Android")
    FBDV = f"{FBMF} {device}"
...
"fb_api_caller_class": "com.facebook.account.login.protocol.Fb4aAuthHandler",
"api_key": "882a8490361da98702bf97a021ddc14d"}
                headers = {'User-Agent': android_user_agent_all_devices(),
'Content-Type': 'application/x-www-form-urlencoded',
'Host': 'graph.facebook.com',
